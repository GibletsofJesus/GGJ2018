...
public class SoundManager : MonoBehaviour
{
    ...
    public void PlaySound(AudioClip sound,float volume = 1,float pitch=1)
    {
        ...
        while (c < audioSrcs.Count){
            ...
            {
                audioSrcs.Add(gameObject.AddComponent<AudioSource>());
                audioSrcs[audioSrcs.Count - 1].clip = sound;
                audioSrcs[audioSrcs.Count - 1].pitch = pitch;
                audioSrcs[audioSrcs.Count - 1].PlayOneShot(sound);
                audioSrcs[audioSrcs.Count - 1].volume = volume * volumeMultiplayer;
            }
            else
            {
                c++;
