...
namespace LeaderBoard
{
    public class HighScores : MonoBehaviour
    {
        ...
        //great job!
        private static HighScores leader = null;
        public static HighScores instance { get { return leader; } }
        string playerName;
        public string fileName;
        int playerScore;
        public int stringLength = 3;
        ScoreData scores = null;// new ScoreData();
        void Awake()
        {
            if (leader == null)
            {
        ...
        public void AddNewScoreToLB(int _score, string _name)
        {
      ...
      public ScoreData GetScoreData()
        {
            if (scores==null)
            {
                if (File.Exists(fileName))
                    scores = SaveFile.Load<ScoreData>(fileName);
                else
                    scores = new ScoreData(fileName);
            }
